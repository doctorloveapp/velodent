...
const quickActionDotClasses: Record<QuickAction, string> = {
  diagnosis: "bg-powder-blue-500",
  hygiene: "bg-powder-blue-500",
  caries: "bg-emerald-400",
  crown: "bg-amber-400",
  endodontics: "bg-violet-400",
  extraction: "bg-red-500",
  mobileProsthesis: "bg-amber-300",
  periodontics: "bg-powder-blue-500"
};
    ...
    <div className="grid gap-3">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="hidden flex-wrap gap-1.5 2xl:flex">
          ...
          {(["caries", "endodontics", "periodontics", "crown", "extraction"] as QuickAction[]).map((action) => (
            <span key={action} className={`rounded-md border px-2 py-1 text-[11px] ${quickActionButtonClasses[action]}`}>
      ...
      <div className="grid gap-3 xl:grid-cols-[minmax(0,1fr)_220px] 2xl:grid-cols-[minmax(0,1fr)_240px]">
        <section className="rounded-md border border-alabaster-grey-500/20 bg-ink-black-950 p-2.5">
          ...
          />
          <div className="my-2 h-px bg-alabaster-grey-500/20" />
          <OdontogramRow
            recordedToothRecords={recordedToothRecords}
            selectedTeeth={selectedTeeth}
            teeth={lowerTeeth}
        ...
        <aside className="rounded-md border border-alabaster-grey-500/20 bg-ink-black-950 p-2.5">
          ...
          <p className="mt-2 font-mono text-[11px] text-alabaster-grey-500">
            {selectedTeeth.length ? `${t("clinicalSelectedTooth")}: ${selectedTeeth.join(", ")}` : t("clinicalSelectTooth")}
          </p>
          {selectedToothRecordInfo ? (
            <div className="mt-2 rounded-md border border-powder-blue-500/25 bg-powder-blue-950/60 p-2">
              <p className="text-[10px] font-semibold uppercase tracking-widest text-pale-sky-500">
          ...
          {selectionMode ? (
            <Button type="button" className="mt-2 h-8 w-full justify-center text-xs" onClick={() => setSelectionMode(false)}>
              {t("mobileFinishSelection")}
            </Button>
          ) : null}

          {activeAction ? (
            <div className="mt-2 rounded-md border border-alabaster-grey-500/20 bg-glaucous-950 p-2">
              <div className="mb-2 flex items-center justify-between gap-2">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-pale-sky-500">
                  {quickActionLabel(activeAction, selectedTeeth.length >= 2, t)}
                </p>
                <Button type="button" variant="secondary" className="h-7 justify-center px-2 text-[11px]" onClick={() => setActiveAction(null)}>
                  {t("mobileCloseMenu")}
                </Button>
              </div>
      ...
      {teeth.map((toothNumber) => {
        ...
        return (
          ...
          >
            ...
            />
            <span className="font-mono text-[9px] font-semibold">{toothNumber}</span>
          </motion.button>
        );
      })}
    </div>
...
function ToothGlyph({ className = "", state, toothNumber }: { className?: string; state: ToothState; toothNumber: number }) {
  ...
  if (position <= 2) {
    return (
      <svg aria-hidden="true" className={`h-5 w-3.5 ${classes}`} fill="none" viewBox="0 0 24 30">
  ...
  if (position <= 5) {
    return (
      <svg aria-hidden="true" className={`h-5 w-4 ${classes}`} fill="none" viewBox="0 0 26 30">
  ...
  return (
    <svg aria-hidden="true" className={`h-5 w-5 ${classes}`} fill="none" viewBox="0 0 30 30">
